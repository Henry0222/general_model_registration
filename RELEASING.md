...
当前 2.0.0 仅完成源码验证，尚未打包或发布。以下为未来发布步骤。
...
6. 核对 ZIP 的 SHA-256。
7. 提交源码，创建并推送标签 `v2.0.0`。
8. 在 GitHub 创建 `v2.0.0` Release，上传 ZIP 和 `.sha256.txt`，不要把二进制提交到 Git 历史。
...
- 第三方许可证和声明与实际打包依赖一致。
- 在打包后的 EXE 中验证资源管理器拖放、套索选区/透选、工作副本删除、浮动面片清理、选区优先配准、失败候选预览和历史 3D 查看器。
