...
当前源码版本：**v2.0.0（源码验证版，未构建 EXE）**。直接运行本目录的 `run_app.bat`。

2.0 默认启用全阶段候选保留与统一评价、多尺度特征、平面结构候选和共同表面精化。每条精化分支都保留父姿态，避免末级拟合覆盖已有好解；共同表面证据不足时输出带警告的整体形状候选。它仍是启发式搜索，不保证找到数学上的全局最优解。

启动窗口不再导入 Open3D / NumPy；计算引擎在首次配准的后台线程中加载。本机源码热缓存测试中，窗口就绪中位时间从 1.483 s 降至 0.259 s。冷启动和打包 EXE 的耗时需要单独测量。
...
下面是既有 1.4.2 下载信息，不是本次 2.0.0 的可执行文件。本轮没有打包或发布。
