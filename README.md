...
当前自动路线按操作者选择采用保守的无 RMS 评分，优先保住原版。它保住了既有面板全部可计算正确初值，但也拒绝了该面板全部有效 B 修正；B 仍可手动选择。具体取舍、连续数值与最新批量界面验证见 [路由复核](docs/V300_ROUTING_QUANTILES.md)。
...
6. 选择配准路线（界面默认自动选择）、结果根目录和偏差参数，点击“开始配准”。批量列表随窗口伸展，左右两栏可拖动调宽。
