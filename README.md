...
当前源码版本：**v3.0.0（源码版，未构建 EXE）**。先按源码安装步骤更新依赖，再运行本目录的 `run_app.bat`。

3.0 加入 A（局部共同表面）和 B（连续表面偏差）几何后级，以及候选对比和实验性自动选择。桌面界面按用户选择默认启用自动路线；公共 API 的 `AlignmentConfig()` 仍默认原版 2.0，以兼容已有调用。自动路线仍属实验功能，A/B 可能使正确位姿退化，原版结果始终可供复核；有人工选区或原版质量失败时跳过后级。使用方式、验证边界和耗时说明见 [3.0 说明](docs/V300_REFINEMENT.md)。
